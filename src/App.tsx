  ...
  if (currentView !== "home") {
    ...
    return (
      <div className="relative bg-black min-h-screen">
        <button onClick={() => setCurrentView("home")} className="fixed top-4 left-4 md:top-8 md:left-8 z-[600] bg-[#F97316] text-black p-3 md:p-4 rounded-2xl font-black uppercase text-[9px] md:text-[10px] tracking-widest shadow-xl hover:scale-110 transition-all min-w-[44px] min-h-[44px]">← Voltar</button>
        {views[currentView]}
      </div>
    );
  }
      ...
      <AnimatePresence mode="wait">
        {showSystem ? (
          ...
          <motion.div key="site" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <nav className="p-4 md:p-6 flex justify-between items-center sticky top-0 z-[100] bg-black/80 backdrop-blur-md border-b-4 border-white/5">
              <h1 className="text-xs md:text-sm font-black tracking-[0.3em] md:tracking-[0.5em] uppercase italic">BELEZA <span className="text-[#F97316]">LINK</span></h1>
            </nav>
            <main className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-12">
              <section className="mb-16 md:mb-24">
                <div className="flex items-center gap-3 md:gap-4 mb-8 md:mb-12">
                  <div className="h-10 md:h-12 w-2 md:w-3 bg-[#F97316] rounded-full"></div>
                  <h2 className="text-3xl md:text-4xl lg:text-6xl font-black uppercase tracking-tighter">NOSSA <br />EXPERIÊNCIA</h2>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                  {cards.map((item) => (
                    <div key={item.n}
                      ...
                      className="bg-zinc-900 p-6 md:p-10 rounded-[20px] md:rounded-[25px] border-l-4 md:border-l-8 border-l-zinc-700 border-b-4 md:border-b-8 border-black shadow-xl cursor-pointer transition-all group hover:border-l-[#F97316] active:scale-95 relative overflow-hidden min-h-[120px]">
                      {item.tag && (
                      ...
                      <h3 className="text-xl md:text-2xl font-black uppercase text-white group-hover:text-[#F97316] transition-colors">{item.n}</h3>
                      <div className="text-[10px] md:text-[12px] text-zinc-400 font-black uppercase mt-4 md:mt-8 group-hover:text-white transition-all">Acessar Bloco →</div>
                    </div>
                  ))}
                </div>
              </section>
              <section className="py-12 md:py-20 text-center">
                <h3 className="text-3xl md:text-5xl lg:text-9xl font-black uppercase tracking-tighter text-white italic mb-8 md:mb-16">MARCAS <span className="text-[#F97316]">ELITE</span></h3>
                <div className="relative h-[280px] md:h-[350px] lg:h-[450px] flex items-center justify-center overflow-hidden" style={{ perspective: isMobile ? "800px" : "1500px" }}>
                  <div className="relative w-full h-full flex items-center justify-center" style={{ transformStyle: "preserve-3d", transform: isMobile ? `rotateX(${rotation}deg)` : `rotateY(${rotation}deg)`, willChange: "transform" }}>
                    {["LUX GLOSS", "SELAGEM 3D", "PLATINUM PRO", "KYHEROS", "VELVET SKIN", "TREEH HAIR", "AKY LISSO", "YGRY", "LAED", "NATURAL LISS"].map((p, i) => (
                      <div key={i} className="absolute w-[140px] sm:w-[180px] md:w-[240px] lg:w-[320px] h-[60px] sm:h-[80px] md:h-[100px] lg:h-[120px] flex items-center justify-center rounded-[15px] md:rounded-[20px] bg-zinc-900 border-b-[4px] md:border-b-[6px] border-r-[4px] md:border-r-[6px] border-black shadow-xl cursor-pointer hover:bg-[#F97316] active:bg-[#F97316] group"
                        style={{ transform: isMobile ? `rotateX(${(i * 360) / 10}deg) translateZ(${isMobile ? '150px' : '200px'})` : `rotateY(${(i * 360) / 10}deg) translateZ(${isMobile ? '300px' : '400px'})`, backfaceVisibility: "hidden" }}>
                        <span className="text-[9px] sm:text-[11px] md:text-base lg:text-xl font-black tracking-widest text-zinc-300 uppercase group-hover:text-black group-active:text-black transition-colors">{p}</span>
                      </div>
                    ))}
                  </div>
                </div>
              </section>
              <footer className="mt-16 md:mt-32 pb-8 md:pb-12 text-center border-t border-white/5 pt-8 md:pt-12">
                <p className="text-[10px] md:text-[12px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-zinc-500">© 2026 Beleza Link</p>
              </footer>
            </main>
          </motion.div>
        )}
