...
import { Share2, X, Copy, MessageSquare, Check, Volume2, VolumeX, Calendar, Bell } from "lucide-react";
...
export default function Live() {
  ...
  const proximasLives = [
    { hora: "20:00", titulo: "Técnica Microblading", dia: "Hoje" },
    { hora: "15:00", titulo: "Gestão de Salão 5.0", dia: "Amanhã" }
  ];
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="aspect-video bg-zinc-900 rounded-[40px] md:rounded-[50px] border border-white/5 relative overflow-hidden shadow-2xl">
             ...
             ></video>
             <button onClick={toggleMute} className="absolute bottom-6 right-6 z-30 bg-black/60 backdrop-blur-md p-4 rounded-full border border-white/10 active:scale-95">
               {isMuted ? <VolumeX size={20} className="text-white/70" /> : <Volume2 size={20} className="text-[#F97316]" />}
             </button>
          </div>

          <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-zinc-900/30 backdrop-blur-xl p-6 rounded-[30px] border border-white/5">
             <div className="flex gap-4">
                {proximasLives.map((live, idx) => (
                ...
                <AnimatePresence mode="wait">
                  {!isSharing ? (
                    ...
                    <motion.div key="menu-sh" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 bg-zinc-800 p-2 rounded-[24px]">
                       <button onClick={() => window.open(`https://wa.me/?text=Assista agora a Live no Beleza Link: ${window.location.href}`)} className="p-4 hover:bg-[#25D366] rounded-xl"><MessageSquare size={18} /></button>
