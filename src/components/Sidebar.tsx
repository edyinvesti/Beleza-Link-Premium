...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-64 bg-black border-r border-white/5 hidden lg:block">
      <div className="p-6 text-amber-500 font-black text-xl">BELEZA LINK</div>
      <nav className="px-4 space-y-2">
        {menuItems.map((item) => (
          <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-4 py-3 rounded-2xl ${location.pathname === item.path ? 'bg-amber-500 text-black font-bold' : 'text-zinc-400 hover:text-white'}`}>
            <item.icon size={20} />
