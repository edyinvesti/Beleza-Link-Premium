    ...
    <div className="p-6 md:p-12 text-white min-h-screen pb-32">
      <header className="mb-12">
        ...
        <h1 className="text-5xl font-black italic tracking-tighter uppercase leading-none">
          CRM <br /> <span className="text-purple-500">Estratégico.</span>
        </h1>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            {vips.length > 0 ? vips.map((vip) => (
              <div key={vip.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-purple-500/50 transition-all group">
                ...
                <button onClick={() => handleWhatsApp(vip.phone, vip.name)} className="bg-purple-500/10 text-purple-500 hover:bg-purple-500 hover:text-white p-3 rounded-xl transition-all">
                  <MessageSquare size={18} />
                </button>
              </div>
            )) : (
              <p className="text-zinc-600 italic text-sm text-center py-10">Nenhum cliente VIP ainda.</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-8 rounded-[2.5rem] text-black group relative overflow-hidden">
            ...
            <button onClick={() => setShowCampaignModal(true)} className="bg-black text-white font-black px-8 py-4 rounded-2xl text-[10px] uppercase tracking-widest flex items-center gap-3 hover:scale-105 transition-all shadow-xl">
              Criar Campanha Agora <ArrowUpRight size={14} />
            </button>
          </div>

          <div onClick={() => navigate("/clientes")} className="bg-zinc-900/50 border border-white/5 p-8 rounded-[2.5rem] flex items-center justify-between cursor-pointer hover:bg-zinc-800 transition-all group">
            <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="bg-zinc-950 border border-white/10 w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-black uppercase italic tracking-tighter">Escolha a <span className="text-amber-500">Ação</span></h2>
