...
export default function Community() {
  ...
  async function handleSendPost(e: React.FormEvent) {
    ...
    if (!error) {
      setNewPost('');
      fetchPosts();
      if (!completedMissions.includes('post')) {
        setCompletedMissions([...completedMissions, 'post']);
      }
    }
  }

  const getLevel = (count: number) => {
    if (count >= 10) return { label: 'OURO', color: 'text-amber-500', icon: Crown, bg: 'bg-amber-500/10' };
    if (count >= 5) return { label: 'PRATA', color: 'text-zinc-300', icon: ShieldCheck, bg: 'bg-zinc-300/10' };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 italic">
          ...
          <div className="bg-zinc-900/50 border border-white/5 p-6 rounded-[2.5rem] shadow-2xl">
            <form onSubmit={handleSendPost} className="flex gap-4">
              <input
                ...
                className="flex-1 bg-black border border-white/5 rounded-2xl px-6 py-4 outline-none focus:border-blue-500 transition-all text-sm"
              />
              <button type="submit" className="bg-blue-500 text-white p-4 rounded-2xl hover:scale-105 transition-all">
                <Send size={20} />
              </button>
            </form>
          </div>
            ...
            {posts.map((post) => (
              <div key={post.id} className="bg-zinc-900/20 border border-white/5 p-8 rounded-[2.5rem] hover:bg-zinc-900/40 transition-all">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-10 h-10 bg-zinc-800 rounded-lg flex items-center justify-center font-black text-blue-400 border border-white/5 text-xs">
                    {post.author_name[0]}
                  </div>
        ...
        <div className="space-y-6 not-italic">
          ...
          <div className="bg-gradient-to-br from-blue-600 to-purple-700 p-8 rounded-[2.5rem] shadow-xl">
            <h3 className="text-white font-black italic uppercase text-lg mb-6 flex items-center gap-2">
              <Award size={20} /> Missões do Dia
            </h3>
